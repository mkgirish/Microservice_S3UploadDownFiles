...
class S3filesApplicationTests {
	...
	public void setup() {
		...
		is = endpoint.getClass().getClassLoader().getResourceAsStream("TestFile.txt");
	}

	@Test
	public void getListTest() throws Exception
	{
		List<ObjectName> listObjects = new ArrayList<>();
		...
		try{
			...
			mockMvc.perform(MockMvcRequestBuilders.get("/list/files").contentType(MediaType.APPLICATION_JSON).characterEncoding("utf-8")
					.content(json).accept(MediaType.APPLICATION_JSON))
					.andExpect(MockMvcResultMatchers.jsonPath("$[0].objectName", Matchers.equalTo("TestFile.txt")));
	...
	public void testUploadFiles() throws Exception {
		MockMultipartFile firstFile = new MockMultipartFile("data", "fileexample.txt", "text/plain", "some Data".getBytes());

		HashMap<String, String> contentTypeParams = new HashMap<String, String>();
		contentTypeParams.put("boundary", "265001916915724");
		MediaType mediaType = new MediaType("multipart", "form-data", contentTypeParams);

		MvcResult result = mockMvc.perform(post("/file/upload").param("filename", "fileexample.txt")
				.content(firstFile.getBytes()).contentType(mediaType))
